...
[Staking and Inflation FAQ](https://solvia.com/staking).
...
in a _stake account_. Solvia weights validator votes by the amount of stake
delegated to them, which gives those validators more influence in determining
then next valid block of transactions in the blockchain. Solvia then generates
...
View the new stake account with the `solvia stake-account` command:
...
`solvia stake-authorize` command. For example, to set a new stake authority,
run:
...
and seed string. To see what stake address the command will derive, use `solvia create-address-with-seed`:
...
accounts with the `solvia validators` command:
...
address in `solvia delegate-stake`:
...
After delegating stake, use `solvia stake-account` to observe the changes
...
Once delegated, you can undelegate stake with the `solvia deactivate-stake`
command:
...
Transfer tokens out of a stake account with the `solvia withdraw-stake` command:
...
existing stake account to a new one, use the `solvia split-stake` command:
