...
```bash
solvia airdrop 1 <RECIPIENT_ACCOUNT_ADDRESS> --url https://api.devnet.solvia.com
```
...
```bash
solvia transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> 0.5 --allow-unfunded-recipient --url https://api.devnet.solvia.com --fee-payer <KEYPAIR>
```
...
Confirm the updated balances with `solvia balance`:
...
```bash
$ solvia-keygen new --outfile my_solvia_wallet.json   # Creating my first wallet, a file system wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
Wrote new keypair to my_solvia_wallet.json
==========================================================================
...
$ solvia airdrop 1 DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.solvia.com  # Airdropping 1 SOL to my wallet's address/pubkey
Requesting airdrop of 1 SOL from 35.233.193.70:9900
1 SOL

$ solvia balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.solvia.com # Check the address's balance
1 SOL

$ solvia-keygen new --no-outfile  # Creating a second wallet, a paper wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
====================================================================
...
$ solvia transfer --from my_solvia_wallet.json 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv 0.5 --allow-unfunded-recipient --url https://api.devnet.solvia.com --fee-payer my_solvia_wallet.json  # Transferring tokens to the public address of the paper wallet
...
with the `solvia transfer` command:
