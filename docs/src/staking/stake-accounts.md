...
A stake account on Solvia can be used to delegate tokens to validators on
...
Stake accounts on Solvia also work differently than those of other Proof-of-Stake
blockchain networks that you may be familiar with. This document describes the
high-level structure and functions of a Solvia stake account.
...
Like other types of accounts on the Solvia network, a stake account that has a
...
Stake account details can be viewed on the [Solvia Explorer](http://explorer.solvia.com/accounts)
