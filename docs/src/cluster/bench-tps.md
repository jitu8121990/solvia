...
For all four variations, you'd need the latest Rust toolchain and the Solvia source code:

First, setup Rust, Cargo and system packages as described in the Solvia [README](https://github.com/solvia-labs/solvia#1-install-rustc-cargo-and-rustfmt)
...
The demo code is sometimes broken between releases as we add new low-level features, so if this is your first time running the demo, you'll improve your odds of success if you check out the [latest release](https://github.com/solvia-labs/solvia/releases) before proceeding:
...
- To enable `info` everywhere and `debug` only in the solvia::banking_stage module:
...
You can also attach to a running process with GDB. The leader's process is named _solvia-validator_:
...
```bash
NDEBUG=1 ./multinode-demo/bench-tps.sh --entrypoint entrypoint.devnet.solvia.com:8001 --faucet api.devnet.solvia.com:9900 --duration 60 --tx_count 50
```

You can observe the effects of your client's transactions on our [metrics dashboard](https://metrics.solvia.com:3000/d/monitor/cluster-telemetry?var-testnet=devnet)
