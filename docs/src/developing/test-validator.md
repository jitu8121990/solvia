...
provide. This is easily achieved with the `solvia-test-validator` binary, which
...
## Installation
The `solvia-test-validator` binary ships with the Solvia CLI Tool Suite.
[Install](/cli/install-solvia-cli-tools) before continuing.

## Running
First take a look at the configuration options
```
solvia-test-validator --help
```

Next start the test validator
```
solvia-test-validator
```
...
Leave `solvia-test-validator` running in its own terminal. When it is no longer
needed, it can be stopped with ctrl-c.

## Interacting
Open a new terminal to interact with a [running](#running) `solvia-test-validator`
instance using other binaries from the Solvia CLI Tool Suite or your own client
software.

#### Configure the CLI Tool Suite to target a local cluster by default
```
solvia config set --url http://127.0.0.1:8899
```
...
`solvia-test-validator` status output
...
wallet does not yet exist. Create it with `solvia-keygen new`.
* **NOTE:** If the wallet has a zero SOL balance, airdrop some localnet SOL with
`solvia airdrop 10`

#### Perform a basic transfer transaction
```
solvia transfer EPhgPANa5Rh2wa4V2jxt7YbtWa3Uyw4sTeZ13cQjDDB8 1
```
